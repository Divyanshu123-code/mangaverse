// src/api/controllers/proxy.controller.js
import axios from "axios";
import { getHeadersForSource } from "../../utils/proxy.utils.js";
  ...
  const tryRequest = async (attempt = 0, refererIdx = 0) => {
    try {
      ...
      if (is403 && refererIdx < refererVariants.length - 1) {
          logger.warn(`🛡️ 403 detected. Trying alternate referer: ${refererVariants[refererIdx + 1]}`);
      ...
      logger.error(`❌ Proxy failed for ${url}: ${error.message}`);
